...
evo_emotes = {
    ...
    "3": "909000075",   # MP40 mx
    "4": "909040010",   # MP40 tc
    "5": "909000081",   # M1014 lt
    "6": "909039011",   # M1014 hh
    "7": "909000063",   # AK
    "8": "909000090",   # Famas
    "9": "909000098",   # UMP
    "10": "909035007",  # M1887
    "11": "909042008",  # Chim Gõ Kiến
    "12": "909041005",  # Groza
    "13": "909033001",  # M4A1
...
# Biểu tượng ánh xạ cho lệnh evo
EMOTE_MAP = {
    1: 909000063,
    2: 909000081,
    3: 909000075,
...
# Giá trị huy hiệu cho các lệnh s1 đến s5 - sử dụng chính xác các giá trị của bạn.
BADGE_VALUES = {
...
def get_player_bio(uid):
    try:
        url = f"https://ffdvinh09-info.vercel.app/player-info?region=SG&uid={uid}"
        res = requests.get(url)
        if res.status_code == 200:
            data = res.json()
...
def send_likes(uid):
    try:
        likes_api_response = requests.get(
             f"https://free-like-api-aditya-ffm.vercel.app/like?uid={uid}&server_name={server_name}&key={BYPASS_TOKEN}",
             timeout=15
             )
        ...
        if likes_api_response.status_code != 200:
            return f"""
[C][B][FF0000]━━━━━
[FFFFFF]Lỗi API tương tự!
Mã trạng thái: {likes_api_response.status_code}
Vui lòng kiểm tra xem uid có chính xác không..
━━━━━
"""
...
"""
        ...
        # Use ThreadPoolExecutor to avoid blocking the async loop
                        ...
                        if inPuTMsG.strip().startswith('/inv '):
                            ...
                            if len(parts) < 2:
                                ...
                                try:
                                    ...
                                    # SUCCESS MESSAGE
                                    success_message = f"[B][C][00FF00]✅ THÀNH CÔNG! Lời mời tham gia nhóm 5 người chơi đã được gửi thành công. {target_uid}!\n"
                                    await safe_send_message(response.Data.chat_type, success_message, uid, chat_id, key, iv)
                        ...
                        if inPuTMsG.startswith(("/6")):
                            ...
                            initial_message = f"[b][ffffff]\n\nHii Đã Tạo Team 6 Tik Tok Admin:@xpowpry.15x Đã Tạo Thành Công Team 6 Free Fire, Vui L̶ồ̶n̶g̶ Chấp Nhận L̶ờ̶i̶ Mời Bot Gửi Tới\n\n"
                            await safe_send_message(response.Data.chat_type, initial_message, uid, chat_id, key, iv)
                            ...
                            # SUCCESS MESSAGE
                            success_message = f"[B][C][00FF00]✅ THÀNH CÔNG! Lời mời tham gia nhóm 6 người chơi đã được gửi thành công đến {uid}!\n"
                            await safe_send_message(response.Data.chat_type, success_message, uid, chat_id, key, iv)

                        if inPuTMsG.startswith(("/3")):
                            # Process /3 command - Create 3 player group
                            initial_message = f"[B][C]{get_random_color()}\n\nTik Tok Admin: @xpowpry.15x Đã Tạo Thành Công Team 3 Free Fire, Vui Lồng Chấp Nhận Lời Mời Bot Gửi Tới\n\n"
                            await safe_send_message(response.Data.chat_type, initial_message, uid, chat_id, key, iv)
                        ...
                        if inPuTMsG.strip().startswith('/roommsg'):
                            ...
                            if len(parts) < 3:
                                ...
                                try:
                                    ...
                                    success_msg = f"[B][C][00FF00]✅ Tin nhắn đã được gửi đến phòng {room_id}!\n"
                                    await safe_send_message(response.Data.chat_type, success_msg, uid, chat_id, key, iv)
                                    print(f"✅ Tin nhắn phòng đã được gửi đến {room_id}: {message}")
                        ...
                        if inPuTMsG.startswith(("/5")):
                            ...
                            initial_message = f"[b][ffffff]\n\nHii Đã Tạo Team 5 Tik Tok Admin:@xpowpry.15x Đã Tạo Thành Công Team 5 Free Fire, Vui L̶ồ̶n̶g̶ Chấp Nhận L̶ờ̶i̶ Mời Bot Gửi Tới\n\n"
                            await safe_send_message(response.Data.chat_type, initial_message, uid, chat_id, key, iv)
...
[FF0000]╠══════════╣
[FFD700]⚡ ADMIN: [FFFFFF]VANTHANGDZS1
[FFD700]✨ YOUTUBE: [FFFFFF] hovanthang198  
[FF0000]╚══════════╝
[FFD700]✨ Nhà phát triển —͟͞͞ </> HOVANTHANG ❄️  ⚡
"""
...
"""
                            ...
                            if len(parts) < 3:
                                ...
                                try:
                                    ...
                                    await safe_send_message(response.Data.chat_type, error_msg, uid, chat_id, key, iv)                                                                                                                                                                                                               
                                                                                          ...
                                                                                          # FIXED JOIN COMMAND
                        if inPuTMsG.startswith('/j'):
                            # Process /j command in any chat type
                            parts = inPuTMsG.strip().split()
                            if len(parts) < 2:
                                error_msg = f"[B][C][FF0000]❌ ERROR! Usage: /j (team_code)\nExample: /j ABC123\n"
                                await safe_send_message(response.Data.chat_type, error_msg, uid, chat_id, key, iv)
                            else:
                                CodE = parts[1]
                                sender_uid = response.Data.uid  # Get the UID of person who sent the command
        
                                initial_message = f"[B][C]{get_random_color()}\nTham gia đội bằng mã: {CodE} \n"
                                await safe_send_message(response.Data.chat_type, initial_message, uid, chat_id, key, iv)
                                ...
                                try:
                                    ...
                                    # SUCCESS MESSAGE
                                    success_message = f"[B][C][00FF00]✅ Thành Công! Gia nhập đội: {CodE}!\n💍 Biểu cảm đã được kích hoạt!\n🤖 Bot bạn♥️ 💕\n"
                                    await safe_send_message(response.Data.chat_type, success_message, uid, chat_id, key, iv)
                        ...
                        if inPuTMsG.strip().startswith('/ghost'):
                            ...
                            if len(parts) < 2:
                                ...
                                CodE = parts[1]
                                initial_message = f"[B][C]{get_random_color()}\nGhost gia nhập đội với mã số: {CodE}...\n"
                                await safe_send_message(response.Data.chat_type, initial_message, uid, chat_id, key, iv)
                        ...
                        if inPuTMsG.strip().startswith('/title'):
                            ...
                            try:
                                ...
                                # SUCCESS MESSAGE
                                success_message = f"[B][C][00FF00]✅ Thành Công! Tiêu đề đã được gửi cho đội hiện tại!\n"
                                await safe_send_message(response.Data.chat_type, success_message, uid, chat_id, key, iv)
                        ...
                        if inPuTMsG.strip().startswith('/e'):
                            ...
                            parts = inPuTMsG.strip().split()
                            if len(parts) < 3:
                                error_msg = f"[B][C][FF0000]❌ LỖI! Cách sử dụng: /e (uid) (emote_id)\nVí dụ: /e 123456789 909000001\n"
                                await safe_send_message(response.Data.chat_type, error_msg, uid, chat_id, key, iv)
                                continue
                                
                            initial_message = f'[B][C]{get_random_color()}\nĐang gửi biểu tượng cảm xúc đến mục tiêu...\n'
                            await safe_send_message(response.Data.chat_type, initial_message, uid, chat_id, key, iv)
                            ...
                            if not s:
                                try:
                                    ...
                                    # SUCCESS MESSAGE
                                    success_msg = f"[B][C][00FF00]✅ THÀNH CÔNG! Biểu tượng cảm xúc {idT} đã gửi tới {len(target_uids)} người chơi(s)!\nMục tiêu: {', '.join(map(str, target_uids))}\n"
                                    await safe_send_message(response.Data.chat_type, success_msg, uid, chat_id, key, iv)

                                except Exception as e:
                                    error_msg = f"[B][C][FF0000]❌ LỖI khi gửi biểu tượng cảm xúc: {str(e)}\n"
                                    await safe_send_message(response.Data.chat_type, error_msg, uid, chat_id, key, iv)
                            else:
                                error_msg = f"[B][C][FF0000]❌ LỖI! Định dạng UID không hợp lệ. Cách sử dụng: /e (uid) (emote_id)\n"
                                await safe_send_message(response.Data.chat_type, error_msg, uid, chat_id, key, iv)
                        ...
                        if inPuTMsG.strip().startswith('/lw'):
                            ...
                            parts = inPuTMsG.strip().split()
                            if len(parts) < 2:
                                error_msg = f"[B][C][FF0000]❌ LỖI! Cách sử dụng: /lw (team_code)\nVí dụ: /lw 123456\n"
                                await safe_send_message(response.Data.chat_type, error_msg, uid, chat_id, key, iv)
                            else:
                                team_code = parts[1]
                                ...
                                if not team_code.isdigit():
                                    error_msg = f"[B][C][FF0000]❌ LỖI! Mã đội chỉ được phép gồm các số.!\nVí dụ: /lw 123456\n"
                                    await safe_send_message(response.Data.chat_type, error_msg, uid, chat_id, key, iv)
                                    continue
                                
                                # Check if already running
                                if auto_start_running:
                                    error_msg = f"[B][C][FF0000]❌ LỖI! Chức năng tự động khởi động đã đang chạy cho nhóm. {auto_start_teamcode}!\nSử dụng lệnh /stop_auto trước khi dừng..\n"
                                    await safe_send_message(response.Data.chat_type, error_msg, uid, chat_id, key, iv)
                                    continue
...
"""
                        ...
                        # EVO CYCLE START COMMAND - /t
                        if inPuTMsG.strip().startswith('/t'):
                            print('Đang xử lý lệnh khởi động chu kỳ evo trong bất kỳ loại trò chuyện nào.')
                            # Declare global variables

                            parts = inPuTMsG.strip().split()
                            uids = []
    
                            # Always use the sender's UID (the person who typed /t)
                            ...
                            if len(parts) > 1:
                                for part in parts[1:]:  # Skip the first part which is "/t"
                            ...
                            # SUCCESS MESSAGE
                            if len(uids) == 1:
                                success_msg = f"[B][C][00FF00]✅ Thành Công!\n🎯 Mục tiêu: Chính bản thân bạn\n🎭 Biểu cảm: Tất cả 20 biểu cảm tiến hóa\n⏰ Độ trễ: 5 giây giữa các biểu cảm\n🔄 Chu kỳ: Vòng lặp liên tục cho đến khi /d\n"
                            else:
                                success_msg = f"[B][C][00FF00]✅ Thành Công!\n🎯 Mục tiêu: Chính bạn + {len(uids)-1} khác người chơi\n🎭 Biểu cảm: Tất cả 20 biểu cảm tiến hóa\n⏰ Độ trễ: 5 giây giữa các biểu cảm\n🔄 Chu kỳ: Vòng lặp liên tục cho đến khi /d\n"
    
                            await safe_send_message(response.Data.chat_type, success_msg, uid, chat_id, key, iv)
                            print(f"Started evolution emote cycle for UIDs: {uids}")
                        
                        # EVO CYCLE STOP COMMAND - /d
                        if inPuTMsG.strip() == '/d':
                            if evo_cycle_task and not evo_cycle_task.done():
                                evo_cycle_running = False
                                evo_cycle_task.cancel()
                                success_msg = f"[B][C][00FF00]✅ THÀNH CÔNG! Chu kỳ biểu cảm tiến hóa đã dừng thành công!\n"
                                await safe_send_message(response.Data.chat_type, success_msg, uid, chat_id, key, iv)
                                print("Evolution emote cycle stopped by command")
                            else:
                                error_msg = f"[B][C][FF0000]❌ LỖI! Không có chu kỳ biểu cảm tiến hóa nào đang hoạt động để dừng lại.!\n"
                                await safe_send_message(response.Data.chat_type, error_msg, uid, chat_id, key, iv)

                        # Fast emote spam command - works in all chat types
                        if inPuTMsG.strip().startswith('/fast'):
                            print('Xử lý nhanh các biểu tượng cảm xúc spam trong mọi loại hình trò chuyện.')
...
# IMPROVED TREE-STYLE HELP MENU SYSTEM (Commands in their original menus) 🌳
                        if inPuTMsG.strip().lower() in ("help", "/help", "menu", "/menu", "commands"):
                            ...
                            # Header
                            header = f"[b][c]{get_random_color()}Chào mừng đến với vthang ˣ ʙᴏᴛ "
                            await safe_send_message(response.Data.chat_type, header, uid, chat_id, key, iv)
                            await asyncio.sleep(0.2)

                            # ───── Group Commands ─────
                            group_commands = """[C][B][00f2ff]═══⚡ LỆNH NHÓM ⚡═══[00f2ff][B]
├─ [b][00f2ff]Team 5
│  └─ [b][00f2ff]/5
├─ [b][00f2ff]Team 6
│  └─ [b][00f2ff]/6
├─ [b][00f2ff]Bot mời người khác
│  └─ [b][00f2ff]/inv [uid]
├─ [b][00f2ff]Mời bot vô team
│  └─ [b][00f2ff]/join [team_code]
├─ [b][00f2ff]Cho bot rời team
│  └─ [b][00f2ff]/exit
[00FFFF]━━━━━━━━━━━━[FF69B4]"""
                            ...
                            # ───── Advanced Commands ─────
                            advanced_commands = """[C][B][ffff00]═══⚡ LỆNH BIỂU CẢM ⚡═══[ffff00][B]
├─ [b][ffff00]Bật hành động từng cái 1
│  └─ [b][ffff00]/e [uid] [id]
├─ [b][ffff00]Spam HĐ 
│  └─ [b][ffff00]/fast [uid] [id]
│  └─ [b][ffff00]Admin
│  ├─ [b][ffff00]/admin
[ffff00]━━━━━━━━━━━━[ffff00]"""
                            ...
                            # ───── Emote Commands ─────
                            emote_commands = """[C][B][0000ff]═══⚡ BIỂU CẢM TIẾN HÓA ⚡═══[0000ff][B]
├─ [b][0000ff]Tự Động Bật S7
│  └─ [b][0000ff]/t [uid] 
├─ [b][0000ff]Dừng Tự Động Bật S7 
├─ [b][0000ff]/d
[b][0000ff]━━━━━━━━━━━━[b][0000ff]"""
                            ...
                            evo_commands = """[C][B][ff0000]═══⚡ NHẮN ADMIN BOT ⚡═══[ff0000][B]
├─ [B][ff0000]TikTok:@xpowpyr.15x
│  └─ [b][ff0000]Dis:jpow15x
└─ [b][ff0000]TikTok:@hovanthang198
   └─ [b][ff0000]BOT TẮT HAY LỖI THÌ NHẮN TAO
[ff0000]━━━━━━━━━━━━[ff0000]"""
                            await safe_send_message(response.Data.chat_type, evo_commands, uid, chat_id, key, iv)
                            await asyncio.sleep(0.2)

                            # ───── AI & Utility Commands ─────
                            ai_commands = """[B][00FFFA]╔═•══•════════════════•══•═╗
[b][FF1493]║ ⚡ [B][FFFF00]BOT THÔNG TIN[b][FFFF00][/B] ⚡
[00FFFA]║
[b][FFFF00]║ 👤 Nhà phát triển: [b][FF1493]HOVANTHANG
[b][32CD32]║ 💻 Trạng thái: [b][32CD32] HOẠT ĐỘNG🟢
[b][1E90FF]║ 🛠 Phiên bản: [b][1E90FF]POWDZ_V2
[b][00FFFA]╚═•══•════════════════•══•═╝"""
                            await safe_send_message(response.Data.chat_type, ai_commands, uid, chat_id, key, iv)
                            await asyncio.sleep(0.2)

                            # ───── Badges Commands ─────
                            badge_commands = """[b][005dff]tiktok:xpowpyr.15x"""
                            await safe_send_message(response.Data.chat_type, badge_commands, uid, chat_id, key, iv)
                            await asyncio.sleep(0.2)
                            
                                                        # ───── Auto Start Commands ─────
                            auto_commands = """[b][ae00ff]Powdzs1"""
                            await safe_send_message(response.Data.chat_type, auto_commands, uid, chat_id, key, iv)
                            await asyncio.sleep(0.2)

                            
                            footer ="""[b][00ff00] ADMIN BOT :H̶ồ̶ ̶V̶ă̶n̶ ̶T̶h̶ắ̶n̶g̶"""
...
async def MaiiiinE():
    Uid , Pw = '4358530983','27FFF5DC5AEEBABCF4C4F067D33F96AA55457C59CA692A98FDFFA050ABA2D34A'
    ...
    os.system('clear')
    print("🔄 Bắt đầu kết nối TCP...")
    print("📡 Đang kết nối với máy chủ Free Fire...")
    ...
    os.system('clear')
    print(render('POWDZ', colors=['white', 'green'], align='center'))
    print('')
    print("🤖 POWDZ BOT - ONLINE")
    print(f"🔹 UID: {TarGeT}")
    print(f"🔹 Name: {acc_name}")
